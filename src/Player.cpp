...
void Player::stopMoving(){
	...
	// Logic statement : if _facing == right, "IdleRight" else "IdleLeft"
	...
	for (int i = 0; i < others.size(); i++){
		...
		if (collisionSide != sides::NONE){
			...
			case sides::TOP:
				...
				if (this->_grounded){
					this->_dx = 0;
					this->_x -= this->_facing == RIGHT ? 0.8f : -0.8f;
				}
				break;
...
void Player::handleSlopeCollisions(std::vector<Slope> &others){
	for (int i = 0; i < others.size(); i++){
		...
		int b = (others.at(i).getP1().y - (others.at(i).getSlope() * fabsf(others.at(i).getP1().x)));
